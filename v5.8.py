...
# -*- coding: utf-8 -*-
"""
WellCorrelator v5.8 — DTW Zone 相似度对比版
- 在 v5.7 基础上增加右键 Zone 区域的 DTW 对比功能
- 自动寻找另一口井中的同名 Zone，计算 DTW 距离与相似度百分比
- 不破坏现有数据模型、保存格式与 UI 交互
"""
...
════════════════════════════════ */
QWidget#sidebar {
    background: #1E293B;   /* 深色基底，提升文字对比度 */
}
...
class GhostObject:
    ...
    def __init__(self, raw_depth, raw_value, anchor_data, color=None, opacity=0.65, label="Ghost"):
        ...
        self.left_bound = self.raw_min   # 左边界显示位置
        self.right_bound = self.raw_max  # 右边界显示位置
    ...
    def get_display_data(self):
        ...
        # 线性映射：将 raw_value 从 [raw_min, raw_max] 映射到 [left_bound, right_bound]
...
class GhostView:
    ...
    def _build(self, plot_item):
        ...
        self._left_handle = pg.InfiniteLine(
            ...
            pen=pg.mkPen(QColor(0,0,0,0), width=5, ),
            movable=True
...
class WellPanel(QWidget):
    ...
    def _on_mouse_hover(self, pos):
        ...
        # 检查深度是否在井数据范围内（一般都在，但做保险）
        if actual_depth < depths.min() or actual_depth > depths.max():
            self.hover_label.setVisible(False)
            return
    ...
    def _redraw_all(self):
        ...
        self.plot_item.setLabel('bottom', f"{self.well.name} - {self._current_curve}")
...
class MainWindow(QMainWindow):
    ...
    def _build_well_controls(self, letter, group_color):
        """构建井控制组，letter='A' 或 'B', group_color 用于边框高亮"""
        refs = {}
        container = QWidget()
        ...
        cmb_fill.setCurrentText("双向")   # 新增：默认双向充填
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("WellCorrelator v5.8 ")
        ...
        hdr_lay.setContentsMargins(14, 11, 14, 11)
        ...
        scroll.viewport().setStyleSheet("background: #1E293B;")

        scroll_w = QWidget()
        scroll_w.setObjectName("scrollContent")
        scroll_w.setStyleSheet("background: #1E293B;")
        ...
        ctr_a, ra = self._build_well_controls("A", "blue")
        ...
        ctr_b, rb = self._build_well_controls("B", "purple")
        ...
        batch_lay.setContentsMargins(6, 8, 6, 8)
        batch_lay.setSpacing(6)

        row_target = QHBoxLayout()
        row_target.setSpacing(6)
        lbl_tgt = QLabel("目标")
        lbl_tgt.setFixedWidth(30)
        ...
        self.text_edit.setPlaceholderText("每行格式: 层名 深度   (支持逗号或空格分隔)\n示例:\nT1  1200.5\nT2, 1350.0")
        ...
        def sync_scale(source, target, offset_to_target=0.0):
            ...
            if new_max > new_min:
                ...
                target.plot_widget.getViewBox().setYRange(new_min, new_max, padding=0.0)
                self._syncing_scale = False
        ...
        if path:
            try:
                ...
                if cols:
                    ...
                    cmb_curve.setCurrentText(default_curve)
    ...
    def _update_fill_btn_colors(self, panel, btn_l, btn_r):
        ...
        btn_l.setStyleSheet(
            ...
            f"color: {'white' if c_l.lightness() < 128 else 'black'}; "
            f"border: 1px solid #475569; border-radius: 5px; padding: 5px 8px; font-size: 11px; font-weight: bold;")
        btn_r.setStyleSheet(
            f"background: rgba({c_r.red()},{c_r.green()},{c_r.blue()},{c_r.alpha()}); "
            f"color: {'white' if c_r.lightness() < 128 else 'black'}; "
            f"border: 1px solid #475569; border-radius: 5px; padding: 5px 8px; font-size: 11px; font-weight: bold;")
